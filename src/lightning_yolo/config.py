...
_DEFAULT_OVERLAP_MULTIPLIER = 5.0
_DEFAULT_CONFIDENCE_MULTIPLIER = 1.0
_DEFAULT_CLASS_MULTIPLIER = 1.0
...
class MatchingConfig:
    ...
    Args:
        algorithm: Which algorithm to use for matching targets to anchors. "simota" (the SimOTA matching rule from
            YOLOX), "tal" (task-aligned top-k matching as used in Ultralytics YOLOv8), "size" (match those prior shapes
            whose width and height relative to the target is below a given ratio), "iou" (match all prior shapes that
            give a high enough IoU), or "maxiou" (match the prior shape that gives the highest IoU, the default).
        ...
        tal_topk: The "tal" algorithm selects up to this many top anchors per target.
...
class LossConfig:
    ...
    Args:
        ...
        label_smoothing: The epsilon parameter (weight) for class label smoothing. 0.0 means no smoothing (binary
            targets), and 1.0 means the target probabilities are always 0.5.
        overlap_multiplier: Overlap loss is scaled by this value. Native default 5.0.
        confidence_multiplier: Confidence loss is scaled by this value. Native default 1.0.
        class_multiplier: Classification loss is scaled by this value. Native default 1.0.
